...
def fk_alg(data, outcome, teacher, dense_controls, 
    ...
    n_teachers = max(data[teacher])
    dummies = (make_dummies(elt) for elt in [teacher] + categorical_controls)
    x = sps.hstack((sps.hstack(dummies), dense_controls))
    b = sps.linalg.lsqr(x, data[outcome].values)
