...
# Swich the role of people and districts everywhere
def reassign(state_df, time_var):
    times = sorted(set(state_df[time_var]))
    last_df = state_df[state_df[time_var] == np.min(times)]
    last_assignments_from_orig = dict(zip(last_df['person'], last_df['distcode']))
    last_assignments_from_sim = dict(zip(last_df['person'], last_df['distcode']))
    last_people = remove_duplicates(last_df['person'])
    #last_districts = remove_duplicates(last_df['distcode'])
    #last_district_from_sim = dict(zip(last_df['person'], last_df['distcode']))
    last_person_from_sim = dict(zip(last_df['distcode'], last_df['person']))
