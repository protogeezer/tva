...
def get_ll_vec_func(n_students_per_class, n_classes, n_students, y_tilde,
                    x_tilde, x_bar, y_bar, teacher_grouped, collin_x_bar_bar,
...
def get_grad_all(sigma_mu_squared, sigma_theta_squared, 
                 sigma_epsilon_squared, beta, lambda_, alpha, 
                 epsilon, ll_vec_func, not_collin_x_bar_bar, 
    ...
    ll_vec_func should have been generated by get_ll_vec_func
    """
    print(not_collin_x_bar_bar)
    print(type(not_collin_x_bar_bar))
    assert (type(not_collin_x_bar_bar) is list) or (type(not_collin_x_bar_bar) is np.ndarray)
    # print('inside gradient')
    # print(sigma_mu_squared, sigma_theta_squared, sigma_epsilon_squared)
    for elt in [sigma_mu_squared, sigma_theta_squared,
                sigma_epsilon_squared, alpha, epsilon]:
    ...
    grad_lambda = np.zeros(beta.shape)
    grad_lambda[not_collin_x_bar_bar] = -(w * x_bar_bar[:, not_collin_x_bar_bar]).T.dot(err)
...
#         assert np.isscalar(elt)
# 
#     vec = np.concatenate(([sigma_mu_squared, sigma_theta_squared, sigma_epsilon_squared], beta, lambda_, [alpha]))
# 
# 
#     def get_hess(i, j):
#         eye = np.eye(len(vec))
...
def get_hess_2(sigma_mu_squared, sigma_theta_squared, sigma_epsilon_squared, 
               beta, lambda_, alpha, epsilon, ll_vec_func, not_collin_x_bar_bar):
