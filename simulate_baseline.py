...
#parameters = {'sd mu':.0135**.5, 'sd theta':.0295**.5, 'mean class size':20, 'mean classes taught':3, 'num teachers': 1000, 'sd epsilon':.2455**.5, 'beta':[2, 3]}
...
def simulate(params):
    ...
    def simulate_one_teacher(teacher_id):
        ...
        data[:, 2] = np.hstack((range(s) for s in sizes))
